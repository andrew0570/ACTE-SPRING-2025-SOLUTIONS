...
int bruteForceGrowth(int n); //scroll down to see a brute force implementation this is not acutually needed for the best solution
...
int growth(int i) //the growth function for branches mod 2 to prevent too alrge numbers from being returned.
...
void solve()
{
    ...
    //1 represents odd and 0 represents even -> this keeps the memory of the variables small and eliminates risk of overflow
    for(int i=0; i<=n; i++)
    {
        y=growth(i);
        z=i%2;
...
int bruteForceGrowth(int n)//this is the brute force calculation of the number of branches at t = n
{ 
    ll branches=1;
    for(int i=1; i<=n; i++)
    {
        branches=branches+(branches*(growth(i)))+i;//remove the mod 2 in the growth function to get the actual number of branches   
    }
    return branches;
}
